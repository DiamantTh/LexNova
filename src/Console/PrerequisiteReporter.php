...
{
    /**
     * @param array{checks: list<array{label:string, ok:bool, value:?string, required:bool}>, blocked: bool} $result
     */
    public function render(OutputInterface $output, array $result): void
    {
        ...
        foreach ($result['checks'] as $check) {
            $status = $check['ok']
                ? '<info>OK</info>'
                : ($check['required'] ? '<error>MISSING</error>' : '<comment>RECOMMENDED</comment>');
            $rows[] = [
                $check['label'],
                $status,
                $check['value'] ?? '—',
